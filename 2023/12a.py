...
@cache
def count(condition: str, groups: tuple[int], cur_grp_len: int, final: str):
    if not condition:
    ...
    if condition[0] == '.':
        if cur_grp_len:
            return 0
        return count(condition[1:], groups, cur_grp_len, final + '.')
    if condition[0] == '#':
        if not groups:
            return 0
        cur_grp_len += 1
    ...
    if condition[0] == '?':
        return (count('.' + condition[1:], groups, cur_grp_len, final) +
                count('#' + condition[1:], groups, cur_grp_len, final))
    raise Exception('END')
...
def main(input_file: str):
    ...
    return sum(count(springs, nums, 0, '') for springs, nums in records)
