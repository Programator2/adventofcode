...
def main(input_file: str):
    ...
    # To do it most effectively, we would have to find a scanner that needs
    # fewest transformations. We're not doing that here, we are selecting
    # scanner 0 as fixed point.

    # Transform all beacons to scanner 0
    beacons = set()
    for i, scanner in enumerate(res):
        for beacon in scanner:
            t = do_transform(i, 0, trans_indirect, trans_ins, beacon)
            beacons.add(t)
    out = len(beacons)
    return out
